...
def set_static_parameters():
    ...
    if SLOW_PTS_MODE:
        pass
        #pts_probe.set_mode("Slow Control")
        #pts_rydberg.set_mode("Slow Control")
        #pts_probe.program_single_freq(PROBE_FREQ)
        #pts_rydberg.program_single_freq(RYDBERG_FREQ)
...
def ramp_from_previous_value(output, start_time, duration, final_value,
                             initial_value=None, samplerate=None,
                             truncation=1.0):
    ...
    if samplerate is None:
        samplerate = default_ramp_samplerate
