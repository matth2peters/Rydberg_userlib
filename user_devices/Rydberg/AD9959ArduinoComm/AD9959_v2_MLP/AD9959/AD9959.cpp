...
void AD9959::initialize(unsigned long sysClk)
{
  ...
  // make the phase accumulator autoclear whenver IOUpdate is done
  // this makes the phase determininstic from shot-to-shot
  _statusCFR[2] = 0x04; 
  byte buffer[3];
  buffer[0] = _statusCFR[0];
  buffer[1] = _statusCFR[1];
  buffer[2] = _statusCFR[2];
